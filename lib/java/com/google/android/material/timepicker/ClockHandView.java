...
class ClockHandView extends View {
  ...
  private static final int SNAP_MODE_NONE = 0;
  private static final int SNAP_MODE_TO_STOPS = 1;
  private static final int SNAP_MODE_TO_NUMBER_STOPS = 2;

  @IntDef({
      SNAP_MODE_NONE,
      SNAP_MODE_TO_STOPS,
      SNAP_MODE_TO_NUMBER_STOPS
  })
  ...
  public void setHandRotation(@FloatRange(from = 0f, to = 360f) float degrees, boolean animate) {
    setHandPosition(degrees, getCurrentLevel(), SNAP_MODE_NONE, animate);
  }

  private void setHandRotationInternal(
      @FloatRange(from = 0f, to = 360f) float degrees, boolean animate) {
    setHandPositionInternal(degrees, getCurrentLevel(), SNAP_MODE_NONE, animate);
  }
  ...
  protected void onDraw(Canvas canvas) {
    ...
    canvas.drawLine(centerX, centerY, selectorCenterX, selectorCenterY, paint);
  ...
  public boolean onTouchEvent(MotionEvent event) {
    ...
    switch (action) {
      case MotionEvent.ACTION_DOWN:
        ...
        @SnapMode int mode = isBeingDragged ? SNAP_MODE_NONE : SNAP_MODE_TO_NUMBER_STOPS;
        setHandPositionFromXY(x, y, mode, /* animate= */ false);
        break;
      case MotionEvent.ACTION_MOVE:
        isBeingDragged |= isOutsideTapRegion(x, y, downX, downY);
        if (isBeingDragged) {
          setHandPositionFromXY(x, y, SNAP_MODE_NONE, /* animate= */ false);
        }
        break;
      ...
      case MotionEvent.ACTION_CANCEL:
        isBeingDragged |= isOutsideTapRegion(x, y, downX, downY);
        if (isBeingDragged) {
          setHandPositionFromXY(x, y, SNAP_MODE_NONE, animatingOnTouchUp);
        }
